...
export default function SearchInput() {
    ...
    const handleSubmit = (event) => {
        ...
        if (textFieldValue != ''){
            axios.post("http://localhost:8080/addQuary",{
                quary: textFieldValue
              }).then(()=>{        
            navgiate('/Search/' + textFieldValue);
        })
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <Autocomplete className='inputContainer'
                ...
                id="free-solo-2-demo"
                disableClearable
                options={searchResults.map((option) => option.des)}
                renderInput={(params) => (
                    <TextField className='input' onKeyPress={(event) => {
